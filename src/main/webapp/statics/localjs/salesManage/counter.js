//查看柜台详情信息
function modify(counterId) {
    window.location.href="/counterModify?counterId=" + counterId;
};
/*
 *分页显示销售营业额列表
 */
                ...
                if(result.data.list.length>0){
                    for (var i = 0; i< result.data.list.length; i++) {
                        ...
                        pageStr+= '<td class="center">'+result.data.list[i].salePrice+'</td>' +
                            ...
                            '<td class="center">'+result.data.list[i].ganame+'</td>'+
                            '<td class="center"><a href="javascript:;" class="edit" onclick="modify('+result.data.list[i].counterId+')">编辑</a></td>';
                        pageStr+='</tr>';
                    }
                }

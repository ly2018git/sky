 ...
 *分页显示销售营业额列表
 */
function getdrugSalesGpsInfoPage(pageIndex){
    //获取各种条件
    //药品名称
    ...
    jQuery.ajax({
        type:"post",
        url:"/gps/getdrugSalesGpsInfoPage",
        dataType:"json",
...
//上一页
    jQuery("#prevPage").click(function(){
        ...
        getdrugSalesGpsInfoPage(pageIndex-1>=1?pageIndex-1:1);
...
//下一页
    jQuery("#nextPage").click(function(){
        ...
        var pageIndex=parseInt($("#hid_pageIndex").val());
        getdrugSalesGpsInfoPage(pageIndex+1<=pages?pageIndex+1:pages);

    });
//末页
    jQuery("#endPage").click(function(){
        var pages=parseInt($("#hid_totalPageCount").val());
        getdrugSalesGpsInfoPage(pages);
    });
